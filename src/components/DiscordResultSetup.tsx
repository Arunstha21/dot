...
import { checkDiscordSetup, fetchDiscordChannels, getDiscordClientId, selectDiscordChannel, setupGuildForStage } from "@/server/actions/discord-actions"
...
export function DiscordSetup({ stageId, onSetupComplete, onGuildChange }: DiscordSetupProps) {
  ...
  `https://discord.com/oauth2/authorize?client_id=${discordClientId}&permissions=1133584&scope=bot`
  ...
  const checkSetup = async () => {
    ...
    try {
      ...
      if (result.setupStep === "ready" && result.guild && result.selectedChannel) {
        ...
        if (onSetupComplete) {
          onSetupComplete(result.guild, result.selectedChannel)
        }
      }
    } catch (error) {
  ...
  const handleGuildSetup = async () => {
    ...
    try {
      ...
      setGuild(result.guild)
      // After guild is added, check if bot is present
      await checkSetup()
    } catch (error) {
